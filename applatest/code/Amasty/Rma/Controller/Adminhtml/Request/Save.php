    ...
    public function execute()
    {
        if ($this->getRequest()->getParams()) {
            ...
            try {
                ...
                if ($this->getRequest()->getParam('status') == '10') {
                    $orderId = (int)$this->getRequest()->getParam('order_id');
                    $order = $this->orderRepository->get($orderId);
                    $orderItemReturnStatus = true;
                    $isNo = false;
                    $isYes = false;
                    foreach ($order->getAllItems() as $item) {
                        if ($item->getRmaReturnStatus() == 'No') {
                            $orderItemReturnStatus = false;
                            $isNo = true;
                        } else {
                            $isYes = true;
                        }
                    ...
                    if ($isNo && $isYes){
                        $order->setReturnStatus('Partial');
                        $order->save();
                    } elseif ($orderItemReturnStatus) {
                        $order->setReturnStatus('Yes');
                        $order->save();
                    }
                }
    ...
    public function processItems(\Amasty\Rma\Api\Data\RequestInterface $model, $items)
    {
        ...
        foreach ($currentRequestItems as $currentRequestItem) {
            ...
            foreach ($items as $item) {
                if (
                    !empty($item[0]) && !empty($item[0][RequestItemInterface::REQUEST_ITEM_ID])
            ...
            if ($currentItems) {
                ...
                foreach ($currentItems as $currentItem) {
                    ...
                    if (
                        !empty($currentItem->getData(RequestItemInterface::REQUEST_ITEM_ID))
                        && ($requestItem = $currentRequestItem[$currentItem->getData(RequestItemInterface::REQUEST_ITEM_ID)])
                    ) {
                        if ($currentItem->getData('status') == '3') {
                            $itemId = $currentItem->getOrderItemId();
                            $orderItem = $this->orderItemRepository->get($itemId);
                            $orderItem->setRmaReturnStatus('Yes');
                            $orderItem->save();
                        }
