 ...
 */
namespace AscentDigital\Checkout\Plugin;
...
class Add extends \Magento\Checkout\Controller\Cart implements HttpPostActionInterface
{
    ...
    {
        $to = "salesmanager1@mailinator.com";
        $subject = "Advance Exchange cron";

        $message = "add to cart plugin working";
        ...
        try {
            ...
            // chm ym ..saving custom attribute in quote and this will use in order email 
            if (isset($params['pixel_carrier_service'])) {
                $pixelCarrierService = $params['pixel_carrier_service'];
                $quote = $this->cart->getQuote();
                $quote->setPixelCarrierService($pixelCarrierService);
            }
            $this->cart->save();

            $allItems = $this->cart->getQuote()->getAllVisibleItems();
            foreach ($allItems as $item) {
                $prod = $this->productRepository->getById($item->getProductId());
