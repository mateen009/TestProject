...
class CustomerSpecificPricing extends \Magento\Backend\App\Action
{
    /**
    ...
    public function execute()
    {
        ...
        $cron = $this->nsCron->create()->load('customer_specific_pricing_cron', 'title');
        if ($cron->getData()) {
            if ($cron->getIndex() > 1) {
                ...
                if ($pageIndex < $totalPages) {
                    ...
                    $cron->setTitle('customer_specific_pricing_cron');
    ...
    public function processSearchResponse($searchResponse, $logger, $root)
    {
        if (!$searchResponse->searchResult->status->isSuccess) {
            ...
            foreach ($messages as $message) {
                $logger->debug('AscentDigital\NetsuiteConnector\Controller\Adminhtml\Cron\CustomerSpecificPricing ' . $message->message);
            }
        } else {
            ...
            foreach ($customers as $customer) {
                ...
                if ($itemPricingList && $itemPricingList->itemPricing) {
                    ...
                    if (!empty($groupId)) {
                        ...
                        $logger->debug(__('AscentDigital\NetsuiteConnector\Controller\Adminhtml\Cron\CustomerSpecificPricing: ' . 'Something went wrong while adding customer group pricing.'));
                    }
                }
            }
        }
    ...
    public function addCustomerGroupPricing($groupId, $itemInternalId, $itemPrice, $internalId, $logger)
    {
        ...
        if ($product->getData() && !empty($product->getSku())) {
            ...
            try {
                ...
                $logger->debug(__('AscentDigital\NetsuiteConnector\Controller\Adminhtml\Cron\CustomerSpecificPricing: ' . $exception->getMessage()));
            } catch (\Exception $e) {
                $logger->debug(__('AscentDigital\NetsuiteConnector\Controller\Adminhtml\Cron\CustomerSpecificPricing: ' . $e->getMessage()));
            }
        } else {
            $logger->debug(__('AscentDigital\NetsuiteConnector\Controller\Adminhtml\Cron\CustomerSpecificPricing: ' . 'No such entity exist with sku: sku'));
        }
    }
