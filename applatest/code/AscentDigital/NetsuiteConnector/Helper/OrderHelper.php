...
class OrderHelper extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    */
    public function createOrder($orderInfo, $customer) {
        // echo "<pre>";print_r($orderInfo);
        // $paymentMethod = $orderInfo['payment_method'];//payment method
        // $shipMethod = $orderInfo['ship_method'];//shipping method
        try{
            ...
            $websiteId = 1;//$this->storeManager->getStore()->getWebsiteId();
            ...
            if(!$customer->getId()){
                ...
                ->setNsInternalId($orderInfo['customer_internal_id']);
                $customer->save();
            }
            ...
            $order->setNsSoNumber($orderInfo['ns_so_number']);
            $order->setNsInternalId($orderInfo['ns_internal_id']);
            $order->setOrderBy('Netsuite');
            $order->save();
