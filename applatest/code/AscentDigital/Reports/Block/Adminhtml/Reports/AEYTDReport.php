...
class AEYTDReport extends \Magento\Backend\Block\Template
{
    private $resource;
    protected $request;
    ...
    public function __construct(
        ...
        \Custom\AdvanceExchange\Model\AdvancedExchange $customFactory,
    ...
    public function getAllRecords()
    {
        ...
        $paramDate=$this->request->getParam('calendar_inputField');
        // print_r($paramDate);die('mateeen');
        //getting collection
        $results = $this->getRecordCollection();
        //apply filter of internal id
        // $results->addFieldToFilter('internalid', $internalId);
        $customDate=date('Y-01-01 h:i:s');
        $results->addFieldToFilter('createddate',array('from' => $customDate,'to' => date('Y-m-d h:i:s')));
        //condition for exchange type filter
        if(isset($paramDate)){
            $results->addFieldToFilter('createddate',array('from' =>  date("Y-m-d", (int)strtotime($paramDate)),'to' => date('Y-m-d h:i:s')));
        }
    ...
    public function getAERecords()
    {
        ...
        $paramCaseStatus=$this->request->getParam('case_status');
        $paramApprovalStatus=$this->request->getParam('approval_status');
        $paramEmail=$this->request->getParam('email_inputField');
        //getting collection
        $results = $this->getAllRecordCollection();
        //apply filter of internal id
        // $results->addFieldToFilter('internalid', $internalId);
        //condition for exchange type filter
        if (isset($paramsData) && $paramsData != 'all') {
...
public function getRecordCollection(){
    $result = $this->customFactory->getCollection();
    return $result;
}
public function getAllRecordCollection(){
    $result = $this->customFactory->getCollection()
