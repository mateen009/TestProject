...
class TotalSalesRepOrdersReport extends \Magento\Sales\Block\Order\History
{
    ...
    public function __construct(
        ...
        \AscentDigital\Reports\Helper\ExportSalesOrderReport $exportHelper,
    ...
    $storeId = $storeManager->getStore()->getStoreId();
        ...
        if($orderFrom!='' && $orderTo!='' && $email!='' && $skus!='' && $salesmanageremail=='' && $teritorymanageremail=='' && $executivemanageremail==''){
           $this->orders = $this->getOrderCollectionFactory()->create()->addFieldToSelect(
            ...
            $this->orders->addFieldToFilter('order_item.sku', array(array('regexp' => $skus)))->addFieldToFilter('main_table.store_id', $storeId)->addExpressionFieldToSelect('total_qty_ordered', 'SUM({{total_qty_ordered}})', 'total_qty_ordered')->setOrder('main_table.customer_ts','desc');
            $this->orders->getSelect()->group('main_table.customer_email');
           ...
           $this->orders = $this->getOrderCollectionFactory()->create()->addFieldToSelect(
                '*'
                    ...
                    'status',
                    ['in' => $orderStatus]
                )->addFieldToFilter('main_table.store_id', $storeId)->addExpressionFieldToSelect('total_qty_ordered', 'SUM({{total_qty_ordered}})', 'total_qty_ordered')->addAttributeToFilter('customer_email',['in' => $email])
                ->setOrder(
                    'customer_ts',
                    'desc'
                );
            ...
            $this->orders = $this->getOrderCollectionFactory()->create()->addFieldToSelect(
                ...
                ->addFieldToFilter(
                    'status',
                    ['in' => $orderStatus]
                )->addExpressionFieldToSelect('total_qty_ordered', 'SUM({{total_qty_ordered}})', 'total_qty_ordered')->addFieldToFilter('main_table.store_id', $storeId)
                ->setOrder(
                    'customer_ts',
                    'desc'
                );
           ...
           $this->orders = $this->getOrderCollectionFactory()->create()->addFieldToSelect(
               ...
               ->addFieldToFilter(
                   'status',
                   ['in' => $orderStatus]
               )->addExpressionFieldToSelect('total_qty_ordered', 'SUM({{total_qty_ordered}})', 'total_qty_ordered')->addFieldToFilter('main_table.store_id', $storeId)->addAttributeToFilter('customer_email',['in' => $email])->setOrder('customer_ts','desc');
            $this->orders->getSelect()->group('main_table.customer_email');
            ...
            $this->orders->getSelect()->join(array('order_item' => 'sales_order_item'),'main_table.entity_id = order_item.order_id');
            $this->orders->addExpressionFieldToSelect('total_qty_ordered', 'SUM({{total_qty_ordered}})', 'total_qty_ordered')->addFieldToFilter('order_item.sku', array(array('regexp' => $skus)))->addFieldToFilter('main_table.store_id', $storeId)->setOrder('main_table.customer_ts','desc');
            $this->orders->getSelect()->group('main_table.customer_email');
        }
            ...
            $this->orders->getSelect()->join(array('order_item' => 'sales_order_item'),'main_table.entity_id = order_item.order_id');
            $this->orders->addFieldToFilter('order_item.sku', array(array('regexp' => $skus)))->addExpressionFieldToSelect('total_qty_ordered', 'SUM({{total_qty_ordered}})', 'total_qty_ordered')->addFieldToFilter('main_table.store_id', $storeId)->setOrder('main_table.customer_ts','desc');
            $this->orders->getSelect()->group('main_table.customer_email');
        }else if($orderFrom=='' && $orderTo=='' && $email=='' && $skus=='' && $salesmanageremail=='' && $teritorymanageremail=='' && $executivemanageremail==''){
            ...
            $this->orders = $this->getOrderCollectionFactory()->create()->addFieldToSelect(
                '*'
                    ...
                    'status',
                    ['in' => $orderStatus]
                                )->addFieldToFilter('main_table.store_id', $storeId)->addExpressionFieldToSelect('total_qty_ordered', 'SUM({{total_qty_ordered}})', 'total_qty_ordered');
            $this->orders->getSelect()->group('main_table.customer_email');
            ...
            $this->orders = $this->getOrderCollectionFactory()->create()->addFieldToSelect(
                '*'
                    ...
                    'status',
                    ['in' => $orderStatus]
                )->addExpressionFieldToSelect('total_qty_ordered', 'SUM({{total_qty_ordered}})', 'total_qty_ordered')->addAttributeToFilter('main_table.sm_email',['in' => $salesmanageremail])->addFieldToFilter('main_table.store_id', $storeId)
                ->setOrder(
            ...
            $this->orders = $this->getOrderCollectionFactory()->create()->addFieldToSelect(
                '*'
                    ...
                    'status',
                    ['in' => $orderStatus]
                )->addExpressionFieldToSelect('total_qty_ordered', 'SUM({{total_qty_ordered}})', 'total_qty_ordered')->addAttributeToFilter('main_table.tm_email',['in' => $teritorymanageremail])->addFieldToFilter('main_table.store_id', $storeId)
                ->setOrder(
                    'main_table.customer_ts',
                    'desc'
                );
            ...
            $this->orders = $this->getOrderCollectionFactory()->create()->addFieldToSelect(
                '*'
                    ...
                    'status',
                    ['in' => $orderStatus]
                )->addExpressionFieldToSelect('total_qty_ordered', 'SUM({{total_qty_ordered}})', 'total_qty_ordered')->addAttributeToFilter('main_table.em_email',['in' => $executivemanageremail] )->addFieldToFilter('main_table.store_id', $storeId)
                ->setOrder(
                    'main_table.customer_ts',
                    'desc'
                );
