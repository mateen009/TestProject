...
class SendDemoOrderEmail extends Action
{
           ...
           foreach($customerObj as $customerObjdata) {
             ...
             if($customer->getData('Customer_Type')==3){
                 if(!empty($this->generatePdf($customer->getData('entity_id'),$customer->getData('Customer_Type')))){
                    ...
                    ->addAttachment(file_get_contents($this->generatePdf($customer->getData('entity_id'),$customer->getData('Customer_Type'))),'Order Demo Report' ,'application/pdf')->setFrom($fromdata)->addTo($to)->getTransport();
                   $transport->sendMessage();
                                           echo "message sent successfully with attachment";
                   $pdf = $mediapath.'/ascent_pdf/'.$customer->getData('entity_id').' OrderDmeoReport.pdf';
                     //After sending item order pdf to sales manager delete it from directory media/ascent_pdf
                   if(file_exists($pdf)) {
                   unlink($pdf);
                   }
                }
    ...
    public function generatePdf($customerId,$customerType){
          ...
          //        generater sales manager wise  demo order pdf and save it in media/ascent_pdf then send this pdf to sales manager.
        
            $orderStatus = ['pending', 'fraud', 'closed', 'holded', 'complete', 'canceled'];
        ...
        if(!empty($orders->getData())){
            $pdf = new \TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
                ...
                $pdf->SetTitle('Order Demo Report');
                $pdf->SetSubject('Order Demo Report');
                $pdf->SetKeywords('Order Demo Report');
                ...
                $html='<h1>Demo Orders Report</h1>';
            $html .= '<table style="font-family: Arial, Helvetica, sans-serif;border-collapse: collapse;width: 100%;"> <tr>
                    <th  style="width:20%;height:30px;border: 1px solid #ddd;padding: 8px;padding-top: 12px;padding-bottom: 12px;text-align: left;background-color: black;color:white;">Order #</th>
                     <th style="width:20%;border: 1px solid #ddd;height:30px;padding: 8px;padding-top: 12px;padding-bottom: 12px;text-align: left;background-color: black;color:white;"> Date</th>
                    ...
                    <th style="width:20%;border: 1px solid #ddd;height:30px;padding: 8px;padding-top: 12px;padding-bottom: 12px;text-align: left;background-color: black;color:white;">SO #</th>
                   </tr>';
                   foreach ($orders as $order) {
                   $html .= '
                          <tr>
                            
                            <td style="width:20%;height:30px;border: 1px solid #ddd;padding: 8px;">'.$order->getIncrementId().'</td>
                            ...
                            <td style="width:20%;border: 1px solid #ddd;height:30px;padding: 8px;">'.$order['ns_so_number'].'</td>
                          </tr>';
                   }
                   $html .= '</table>';
             ...
             $directory = $this->objectManager()->get('\Magento\Framework\Filesystem\DirectoryList');
             $mediapath = $directory->getPath('media');
             $pdf->Output($mediapath.'/ascent_pdf/'.$customerId.' OrderDmeoReport.pdf', 'F');
            return $baseurl.'pub/media/ascent_pdf/'.$customerId.' OrderDmeoReport.pdf';
        }
