...
class SendOrderDetailReportEmail extends Action
{
           ...
           foreach($customerObj as $customerObjdata) {
             ...
             if($customer->getData('Customer_Type')==3){
                 if(!empty($this->generatePdf($customer->getData('entity_id'),$customer->getData('Customer_Type')))){
                     $filePath = $baseUrl.'pub/media/';
                    $fileName = "OrderStatusReport.pdf";
                    ...
                    ->addAttachment(file_get_contents($this->generatePdf($customer->getData('entity_id'),$customer->getData('Customer_Type'))),'Order Detail Report' ,'application/pdf')->setFrom($fromdata)->addTo($to)->getTransport();
                   $transport->sendMessage();
                                           echo "message sent successfully with attachment";
                   $pdf = $mediapath.'/ascent_pdf/'.$customer->getData('entity_id').' OrderDetailReport.pdf';
                     //After sending sales order pdf to sales manager delete it from directory media/ascent_pdf
                   if(file_exists($pdf)) {
                   unlink($pdf);
                   }
                }
    ...
    public function generatePdf($customerId,$customerType){
          ...
          //        generater sales manager wise sales orders pdf and save it in media/ascent_pdf then send this pdf to sales manager.
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $storeManager = $objectManager->get('\Magento\Store\Model\StoreManagerInterface');
        $baseUrl = $storeManager->getStore()->getBaseUrl();
        ...
        if(!empty($orders->getData())){
            $pdf = new \TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
                ...
                $pdf->SetTitle('Order Detail Report');
                $pdf->SetSubject('Order Detail Report');
                $pdf->SetKeywords('Order Detail Report');
                ...
                $heading='<h1>Order Detail Report</h1>';
                ...
                foreach ($orders as $order) {
                    ...
                    <tr>
                    <td><span STYLE="font-weight:bold">Increment Id #  '.$order->getIncrementId().'</span></td>
                    ...
                    <tr>
                    <td><span>Customer Email:  '.$order->getCustomerEmail().'</span></td>
                    </tr>
                    <tr>
                    <td></td>
                    </tr>
                   ...
                   $html .= '<table style="font-family: Arial, Helvetica, sans-serif;border-collapse: collapse;width: 100%;"> <tr>
                     <th  style="width:40%;height:30px;border: 1px solid #ddd;padding: 8px;padding-top: 12px;padding-bottom: 12px;text-align: left;background-color: black;color:white;">Product Name</th>
                     <th style="width:25%;border: 1px solid #ddd;height:30px;padding: 8px;padding-top: 12px;padding-bottom: 12px;text-align: left;background-color: black;color:white;">Sku</th>
                     <th style="width:20%;border: 1px solid #ddd;height:30px;padding: 8px;padding-top: 12px;padding-bottom: 12px;text-align: left;background-color: black;color:white;">Sub Total</th>
                   </tr>';
                   ...
                   $html .= '
                          <tr>
                            ...
                            <td style="width:25%;border: 1px solid #ddd;height:30px;padding: 8px;">'.$item->getSku().'</td>
                            <td style="width:20%;border: 1px solid #ddd;height:30px;padding: 8px;">'.'$'.number_format($item->getRowTotal(),2).'</td>
                          </tr>';
                   }
                        $html .= '<tfoot>
                     ...
                     <tr>
                       ...
                       <td><span STYLE="font-weight:bold">'.'$'.number_format($order->getGrandTotal(),2).'</span> </td>
                     </tr>
                   </tfoot>';
                   $html .= '</table>';
             ...
             $directory = $this->objectManager()->get('\Magento\Framework\Filesystem\DirectoryList');
             $mediapath = $directory->getPath('media');
             $pdf->Output($mediapath.'/ascent_pdf/'.$customerId.' OrderDetailReport.pdf', 'F');
            return $baseUrl.'pub/media/ascent_pdf/'.$customerId.' OrderDetailReport.pdf';
        }
