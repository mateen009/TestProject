...
class SendOrderFilterByItemReportEmail extends Action
{
            ...
            if($customer->getData('send_customers_email')=='1'){
             if($customer->getData('Customer_Type')==3){
                 if(!empty($this->generatePdf($customer->getData('entity_id'),$customer->getData('Customer_Type')))){
                    ...
                    ->addAttachment(file_get_contents($this->generatePdf($customer->getData('entity_id'),$customer->getData('Customer_Type'))),'Order Item Report' ,'application/pdf')->setFrom($fromdata)->addTo($to)->getTransport();
                   $transport->sendMessage();
                                           echo "message sent successfully with attachment";
                   $pdf = $mediapath.'/ascent_pdf/'.$customer->getData('entity_id').' OrderItemReport.pdf';
    ...
    public function generatePdf($customerId,$customerType){
          ...
          //        generater sales manager wise  orders item pdf and save it in media/ascent_pdf then send this pdf to sales manager.
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        ...
        if(!empty($orders->getData())){
            $pdf = new \TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
                ...
                $pdf->SetTitle('Order Item Report');
                $pdf->SetSubject('Order Item Report');
                $pdf->SetKeywords('Order Item Report');
                ...
                $html='<h1>Order By Item Report</h1>';
            $html .= '<table style="font-family: Arial, Helvetica, sans-serif;border-collapse: collapse;width: 100%;"> <tr>
             ...
             $directory = $this->objectManager()->get('\Magento\Framework\Filesystem\DirectoryList');
             $mediapath = $directory->getPath('media');
             $pdf->Output($mediapath.'/ascent_pdf/'.$customerId.' OrderItemReport.pdf', 'F');
            return $baseUrl.'pub/media/ascent_pdf/'.$customerId.' OrderItemReport.pdf';
        }
