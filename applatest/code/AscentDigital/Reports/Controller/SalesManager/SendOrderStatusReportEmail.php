...
class SendOrderStatusReportEmail extends Action
{
            ...
            if($customer->getData('send_customers_email')=='1'){
             if($customer->getData('Customer_Type')==3){
                 if(!empty($this->generatePdf($customer->getData('entity_id'),$customer->getData('Customer_Type')))){
                    ...
                    ->addAttachment(file_get_contents($this->generatePdf($customer->getData('entity_id'),$customer->getData('Customer_Type'))),'Order Status Report' ,'application/pdf')->setFrom($fromdata)->addTo($to)->getTransport();
                   $transport->sendMessage();
                                           echo "message sent successfully with attachment";
                   $pdf = $mediapath.'/ascent_pdf/'.$customer->getData('entity_id').' OrderStatusReport.pdf';
    ...
    public function generatePdf($customerId,$customerType){
        ...
        if(!empty($orders->getData())){
            $pdf = new \TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
                ...
                $pdf->SetTitle('Order Status Report');
                $pdf->SetSubject('Order Status Report');
                $pdf->SetKeywords('Order Status Report');
                ...
                $html='<h1>Order Status Report</h1>';
            $html .= '<table style="font-family: Arial, Helvetica, sans-serif;border-collapse: collapse;width: 100%;"> <tr>
                 ...
                 <th style="width:12%;border: 1px solid #ddd;height:30px;padding: 8px;padding-top: 12px;padding-bottom: 12px;text-align: left;background-color: black;color:white;">Order Total</th>
                 <th style="width:13%;border: 1px solid #ddd;height:30px;padding: 8px;padding-top: 12px;padding-bottom: 12px;text-align: left;background-color: black;color:white;">Order Status</th>
                ...
                $html .= '
                       <tr>
                         ...
                         <td style="width:12%;height:30px;border: 1px solid #ddd;padding: 8px;">'.'$'.number_format($order->getGrandTotal(),2).'</td>
                         <td style="width:13%;border: 1px solid #ddd;height:30px;padding: 8px;">'.$order->getStatus().'</td>
             ...
             $directory = $this->objectManager()->get('\Magento\Framework\Filesystem\DirectoryList');
             $mediapath = $directory->getPath('media');
             $pdf->Output($mediapath.'/ascent_pdf/'.$customerId.' OrderStatusReport.pdf', 'F');
            return $baseurl.'pub/media/ascent_pdf/'.$customerId.' OrderStatusReport.pdf';
        }
