...
class SendSalesReportEmail extends Action
{
           ...
           foreach($customerObj as $customerObjdata) {
             ...
             if($customer->getData('Customer_Type')==3){
                 if(!empty($this->generatePdf($customer->getData('entity_id'),$customer->getData('Customer_Type')))){
                    ...
                    $fromEmail = "ascentterritorymanager@gmail.com";
                    $fromName = 'Ascent Digital';
                    $fromdata = ['email' => $fromEmail, 'name' => $fromName];
                    $to =  'ascentterritorymanager@gmail.com';
                    $emailtempalte = 11;
                    ...
                    ->addAttachment(file_get_contents($this->generatePdf($customer->getData('entity_id'),$customer->getData('Customer_Type'))),'Sales Report' ,'application/pdf')->setFrom($fromdata)->addTo($to)->getTransport();
                   $transport->sendMessage();
                                           echo "message sent successfully with attachment";
                   $pdf = $mediapath.'/ascent_pdf/'.$customer->getData('entity_id').' SalesReport.pdf';
    ...
    public function generatePdf($customerId,$customerType){
        ...
        $orders = $orderCollectionFactory->create($this->getManagerReps($customerId,$customerType))->addFieldToSelect('*')->addFieldToFilter('status',array("in",'Shipping','Processing','Complete'))->setOrder('created_at','desc');
        ...
        if(!empty($orders->getData())){
            $pdf = new \TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
                ...
                $html='<h1>Sale By Sales Rep</h1>';
            $html .= '<table style="font-family: Arial, Helvetica, sans-serif;border-collapse: collapse;width: 100%;"> <tr>
                 <th  style="width:20%;height:30px;border: 1px solid #ddd;padding: 8px;padding-top: 12px;padding-bottom: 12px;text-align: left;background-color: black;color:white;">Quote #</th>
             ...
             $directory = $this->objectManager()->get('\Magento\Framework\Filesystem\DirectoryList');
             $mediapath = $directory->getPath('media');
             $pdf->Output($mediapath.'/ascent_pdf/'.$customerId.' SalesReport.pdf', 'F');
            return $baseurl.'pub/media/ascent_pdf/'.$customerId.' SalesReport.pdf';
        }
