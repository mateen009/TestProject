...
class SendTotalSaleBySalesRepEmail extends Action
{
    ...
    public function __construct(
       ...
       public function execute()
       {
           ...
           $templateOptions = [
            ...
            $templateVars = [
            'subject'      => 'Total Sales By Sales Rep Report'
            ];
...
//
                            if(!empty($this->generatePdf($customer->getData('entity_id'),$customer->getData('Customer_Type')))){
                                ...
                                $fileName = "TotalSalesBySalesRepReport.pdf";
                                ...
                                ->addAttachment(file_get_contents($this->generatePdf($customer->getData('entity_id'),$customer->getData('Customer_Type'))),'Total Sale By Sales Rep' ,'application/pdf')->setFrom($fromdata)->addTo($to)->getTransport();
                               $transport->sendMessage();
                                                       echo "message sent successfully with attachment";
                               $pdf = $mediapath.'/ascent_pdf/'.$customer->getData('entity_id').' TotalSalesBySalesRepReport.pdf';
                                 //After sending total sales by sales rep order pdf to sales manager delete it from directory media/ascent_pdf
                               if(file_exists($pdf)) {
                               unlink($pdf);
                               }
                            }
    ...
    public function generatePdf($customerId,$customerType){
          ...
          //        generate sales manager wise  total sales by sales rep pdf and save it in media/ascent_pdf then send this pdf to sales manager.
        ...
        $orders->getSelect()->group('customer_email');
        ...
        if(!empty($orders->getData())){
            $pdf = new \TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
                ...
                $pdf->SetTitle('Total Sales By Sales Rep Report');
                $pdf->SetSubject('Total Sales By Sales Rep Report');
                $pdf->SetKeywords('Total Sales By Sales Rep Report');
                ...
                $html='<h1>Total Sale By Sales Rep</h1>';
            $html .= '<table style="font-family: Arial, Helvetica, sans-serif;border-collapse: collapse;width: 100%;"> <tr>
                 <th  style="width:40%;height:30px;border: 1px solid #ddd;padding: 8px;padding-top: 12px;padding-bottom: 12px;text-align: left;background-color: black;color:white;">Email</th>
                ...
                $html .= '
                       <tr>
                         ...
                         <td style="width:40%;height:30px;border: 1px solid #ddd;padding: 8px;">'.$order->getCustomerEmail().'</td>
                         <td style="width:20%;border: 1px solid #ddd;height:30px;padding: 8px;">'.$this->getOrderCount($order->getCustomerEmail(),$customerId,$customerType).'</td>
             ...
             $directory = $this->objectManager()->get('\Magento\Framework\Filesystem\DirectoryList');
             $mediapath = $directory->getPath('media');
             $pdf->Output($mediapath.'/ascent_pdf/'.$customerId.' TotalSalesBySalesRepReport.pdf', 'F');
            return $baseurl.'pub/media/ascent_pdf/'.$customerId.' TotalSalesBySalesRepReport.pdf';
        }
            
                
    }
    
    public function getOrderCount($customerEmail,$customerId,$customerType){
