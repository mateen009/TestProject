...
class ExportDueBackReport extends AbstractHelper{
    /** @var ResultFactory */
    ...
    public function exportData($orders)
    {
        ...
        foreach ($orders as $order) {
            ...
            $orderData[] = date('M d, Y', strtotime($order->getDueDate()));
            $orderData[] = $order->getStatusLabel();
            $orderData[] = $order->getReturnStatus();
        ...
        $csvfilename = 'Due Back Report.csv';
    ...
    {
        $headers = ['SO #', 'Email', 'Agency Name', 'Due Date', 'Order Status', 'Return Status','Items','Sku'];
        return $headers;
    }
