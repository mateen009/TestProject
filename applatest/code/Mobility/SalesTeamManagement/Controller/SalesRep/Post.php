...
class Post extends \Magento\Framework\App\Action\Action implements HttpPostActionInterface
{
    ...
    public function __construct(
        ...
        \Magento\Customer\Model\ResourceModel\Customer\Collection $customerObj,
    ...
    {
        $customer = $this->customerObj->addAttributeToSelect('id')
                ->addAttributeToFilter('attu_id',$this->getRequest()->getPost('attu_id'))
                ->load();
        $customerId ='';
        foreach($customer as $cus){
            $customerId = $cus->getId();
        }
        if($customerId!=$this->getRequest()->getPost('id')){
            $this->messageManager->addErrorMessage(
                __('Attuid already exists.')
            );
            return $this->resultRedirectFactory->create()->setPath('customer/salesrep/index');
        }else{
             if (!$this->getRequest()->isPost()) {
                    ...
                    try {
                        ...
                        $logger->info('nsInteralId: '.$nsInteralId);
                        $logger->info('Attu Id: '.$attuId);
                        ...
                        $customer->setCustomAttribute('customer_phone', $this->getRequest()->getPost('phone_num'));
                        $customer->setCustomAttribute('customers_company', $this->getRequest()->getPost('company_name'));
                        ...
                        $customer->setCustomAttribute('Customer_Type', 1);
                        $customer->setCustomAttribute('SalesManager_ID', $this->customerSession->getCustomerId());
            ...
            //            // setting territory manager id
                        ...
                        if ($add_new_sales_rep) {
                            $this->messageManager->addSuccessMessage(
                                __('Sales rep added successfully!')
                            );
                        } else {
                            $this->messageManager->addSuccessMessage(
                                __('Sales rep updated successfully!')
                            );
                        }
